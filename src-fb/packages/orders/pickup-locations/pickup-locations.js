...
import { AsyncStorage, FlatList, TouchableWithoutFeedback, Dimensions, Platform, PermissionsAndroid, TouchableOpacity } from "react-native";
import { View, Button, Container, Icon, Title, Spinner, Thumbnail, Text } from 'native-base';
import { Animated, ProviderPropType, Marker, AnimatedRegion } from 'react-native-maps';
        ...
        return (
            <Container style={styles.container}>
                <HeaderComponent
                    ...
                    headerBody={(<Title style={commonStyles.headerIconColor}>Available pickup locations</Title>)}
                />

                {LATITUDE && LONGITUDE ? <Animated
                    showUserLocation
                ...
                >
                    {pickUpLocationsData.map(marker => (
                        <Marker
                            ...
                            // })}
                            onPress={() => this.onClicMarkLocation(LATITUDE, LONGITUDE, marker.location.lat, marker.location.lng, marker)}
                            style={{ justifyContent: 'center', alignItems: 'center', width: 50, height: 50 }}
                        >
                            <Thumbnail circle style={{ width: 50, height: 50 }}
                ...
                {showDetails && this.state.detailPopup ? <AnimatedView>
                    <TouchableOpacity style={{ flex: 0.6 }} onPress={() => this.handleClose()} />
                    <View style={{ backgroundColor: '#fff', flex: 0.4, borderRadius: 15, marginBottom: -10 }}>
                        <View style={{ flexDirection: 'row', flex: 0.2, borderBottomWidth: 1, borderBottomColor: '#dfe6e9' }}>
                            <Button style={{ alignSelf: 'center', elevation: 0, backgroundColor: 'transparent', flex: 0.5 }} onPress={() => this.handleClose()}>
                                <Icon name="close" style={{ fontSize: 34, color: '#000' }} />
                            </Button>
                            <Text style={{ alignSelf: 'center', fontSize: 18 }}>Pickup</Text>
                        </View>
                        <View style={{ paddingHorizontal: 20, flex: 0.6, justifyContent: 'center' }}>
                            <Text style={{ fontSize: 18, fontWeight: 'bold' }}>{this.state.detailPopup.selectedLocation.address}</Text>
                            <View style={{ flexDirection: 'row' }}>
                                <Text style={{ marginRight: 15 }}>{this.state.detailPopup.selectedLocation.name}</Text>
                                <Text>{this.state.detailPopup.distanceFromPin}</Text>
                            </View>
                        </View>
                        <Button onPress={() => this.onSelectPickUpLocation(this.state.detailPopup.selectedLocation)} full style={{ elevation: 0, marginHorizontal: 30, borderRadius: 5 }}>
...
export default connect(mapStateToProps, mapDispatchToProps)(PickUpLocationsContainer);
