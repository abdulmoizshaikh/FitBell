...
const getPickUpLocations = () => (dispatch) => {
    return new Promise((resolve, reject) => {
        async function onRequest() { dispatch(orderActions.getPickUpLocationsRequest()) }
...
const addOrder = (data) => (dispatch) => {
    return new Promise((resolve, reject) => {
        async function onRequest() { dispatch(orderActions.addOrderRequest()) }
        async function onSuccess(response) {
        ...
        onRequest();
        APIService.request({
            method: 'post', url: '/orders', data: data
        }).then(onSuccess).catch(onError);
    })
}
...
const getMyOrders = (token, params) => (dispatch) => {
    return new Promise((resolve, reject) => {
        async function onRequest() { dispatch(orderActions.getMyOrdersRequest()) }
        async function onSuccess(response) {
        ...
        onRequest();
        APIService.request({
            method: 'get', url: '/orders/my', params: params
        }).then(onSuccess).catch(onError);
    })
}
