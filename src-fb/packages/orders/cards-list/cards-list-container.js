...
import { ScrollView, FlatList, AsyncStorage, TouchableWithoutFeedback } from 'react-native';
import { View, Container, Icon, Left, Right, Text, Button, Card, CardItem, Spinner, Thumbnail } from 'native-base';
...
class CardsListContainer extends React.Component {
    ...
    render() {
        ...
        myOrdersData.map((order) => {
            if (new Date(order.date).toISOString().split("T")[0] === new Date().toISOString().split("T")[0]) {
        ...
        return (
            <Container>
                ...
                <HeaderComponent
                    ...
                    headerRight={
                        <View style={styles.headerRightView}>
                            <Icon name="map" type="Feather" style={styles.headerCalenderIconStyle} />
                            <Icon name="calendar" type="Feather"
                                style={styles.headerCalenderIconStyle}
                                onPress={() => this.props.navigation.navigate('CalenderCardsScreen')} />
                ...
                <ScrollView showsVerticalScrollIndicator={false} >
                    ...
                    {/* video tutorial */}
                    {showTutorials ? <ToturialsComponent onClose={this.onCloseTutorials} /> : null}
                    ...
                    {this.props.isFetching ? null :
                        ...
                        <View style={{ marginHorizontal: 20 }}>
                            <Text style={{ fontSize: 24, fontWeight: 'bold', marginVertical: 10 }}>Today</Text>
                            <Thumbnail square large style={styles.noMealIcon} source={images.noMealIcon} />
                            <Text info style={{ marginHorizontal: 50, textAlign: 'center' }}>Looks like you dont have any meal for today</Text>
                        </View>}
...

                    {
                        ...
                        <View style={styles.MV10}>
                            ...
                            <View style={styles.rowView}>
                                <AddMealCard
                                    navigation={this.props.navigation}
                                    _date={this.dateAfterLastOrder(myOrdersData[myOrdersData.length - 1].date.substring(0, 10))}
                                />
                            </View>
                        </View>
                    }
