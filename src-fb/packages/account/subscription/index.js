...
import { Text, Container, Icon, Title, Spinner, View, Card, Right } from 'native-base';
...
class Subscription extends React.Component {
    ...
    updateSubscriptionHandler = async (subscriptionId) => {
        ...
        try {
            let result = await this.props.actions.updateSubscription({ subscription_id: subscriptionId })
            if (result) {
            ...
            if (err.status && err.status === 424) {
                Alert.alert(
                    ...
                    [
                        {
                            text: 'OK', onPress: () => this.props.navigation.navigate("PaymentMethod", {
                                fromComponent: true
                            })
                        },
                    ],
        ...
        return (
            <Container>
                <HeaderComponent
                    ...
                    headerBody={(<Title style={commonStyles.headerIconColor}>Payment Method</Title>)}
                />
                ...
                <ScrollView showsVerticalScrollIndicator={false} >
                    ...
                    <View style={styles.subPkgsTitleWrapper}>
                        <Text style={styles.headerTitle}>Flexible subscription Options</Text>
                    </View>
                    ...
                    {this.props.isFetching && subscriptions.length === 0 ? <Spinner /> :
                        subscriptions.map((item, index) => {
                            return (
                                <SubscriptionPackage item={item}
                                    key={index.toString()}
                    ...
                    <View style={styles.footerSectionWrapper}>
                        <Text style={styles.footerText}>
                            Flexible Subscription Options That Will Save You Money on Stuff You Actually Need.
                             That will more sensible than you pay its individual payment.
                        </Text>
                    </View>
