      ...
      if (restrictionsInUser.length === 0 || allergiesInUser.length === 0) {
        this.props.navigation.dispatch(
          StackActions.reset({
            index: 0,
            actions: [
              CommonActions.navigate({routeName: 'AllergiesRestriction'}),
            ],
          }),
        );
      } else if (
        ...
        this.props.navigation.dispatch(
          StackActions.reset({
            index: 0,
            actions: [CommonActions.navigate({routeName: 'FitnessGoals'})],
          }),
        );
      } else {
        this.props.navigation.dispatch(
          StackActions.reset({
            index: 0,
            actions: [CommonActions.navigate({routeName: 'Menu'})],
          }),
        );
      }
    }
