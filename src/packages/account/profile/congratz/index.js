...
import { AsyncStorage, TouchableWithoutFeedback, TouchableOpacity } from "react-native";
        ...
        return (
            <Container style={styles.containerStyle}>
                ...
                <View style={{ flex: 0.5, justifyContent: 'center', alignItems: 'center', }}>
                    <Text dark style={{ fontSize: 28, fontWeight: 'bold' }}>You’re all set!</Text>
                    <Text info style={{ textAlign: 'center' }} >{`Thanks for registering \nyour account`}</Text>
                </View>
                <VideoTutorials />
                <View style={{ flex: 0.3, justifyContent: 'center', alignItems: 'center' }} >
                    <Text dark style={{ fontSize: 28, fontWeight: 'bold' }}>Quick Tutorials</Text>
                    <Text info style={{ textAlign: 'center' }} >Get started and learn about fitbowl.</Text>
                </View>
                <Button primary full
                    style={{ borderRadius: 5, marginHorizontal: 20, elevation: 0 }}
                    onPress={() => this.onSubmitHandler()}
                >
                    {this.props.isFetching ? <Spinner color="#fff" /> : <Text uppercase={false}>Start Explore</Text>}
                </Button>
            </Container >
        );
    }
