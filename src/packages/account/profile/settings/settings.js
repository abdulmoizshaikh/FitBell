  ...
  constructor(props) {
    ...
    this.state = {
      ...
      routes: [
        {divider: true},
        {
          name: 'Daily Confirmation',
          value: 'DailyConfirmation',
          DailyConfirmation: true,
        },
        {
          name: 'Confirm Order from trainer',
          value: '/',
          Switch: true,
          val: 'setConfirmOrder',
          state: this.props.user.confirm_order,
        },
        {
          name: 'Notifications',
          value: '/',
          Switch: true,
          val: 'setAllowNotification',
          state: this.props.user.allow_notification,
        },
        {divider: true},
  ...
  checkRoutesOnClickHandler = data => {
    if (data.value == '/') {
      ...
      this.props.navigation.dispatch(
        StackActions.reset({
          index: 0,
          actions: [NavigationActions.navigate({routeName: data.value})],
        }),
      );
    }
  };
      ...
      <Container>
        <ScrollView showsVerticalScrollIndicator={false}>
          <HeaderComponent
            ...
            headerBody={
              <Title style={commonStyles.headerIconColor}>Settings</Title>
            }
          />
                  ...
                  <Right>
                    {data.Switch ? (
                      <Switch
                        ...
                        value={data.state}
                        onValueChange={() =>
                          this.onSubmitToggleHandler({val: data.val})
                        }
                      />
                    ) : data.DailyConfirmation ? (
                      <View style={styles.DailyConfirmationRightView}>
                        <Text style={styles.DailyConfirmationRightText}>
                          Mannual
                        </Text>
                        <Icon
                          name="chevron-thin-right"
                          type="Entypo"
                          style={styles.DailyConfirmationRightIcon}
                        />
                      </View>
                    ) : (
                      <Icon
