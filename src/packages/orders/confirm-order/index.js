  ...
  confirmOrder = async () => {
    ...
    try {
      ...
      const orderDetails = {
        product_id: this.props.orderDetails.orderedProductDetails._id,
        location_id:
          isDefault && this.props.user.default
            ? this.props.user.default.delivery_address
            : this.props.orderDetails.locationDetails._id,
        delivery_type:
          isDefault && this.props.user.default
            ? this.props.user.default.delivery_type
            : this.props.orderDetails.deliveryType,
        date: this.props.orderDetails.orderDate,
      };
    ...
    return isLoading ? (
      ...
      <Container>
        ...
        <ScrollView
          ...
          Object.keys(orderedProductDetails).length <= 1 ? (
            <Spinner />
          ) : (
            <View style={StylesCard.IndividualSection}>
              <CardInfoComponent
                ...
                calories={`${(
                  4 * orderedProductDetails.carbs +
                  9 * orderedProductDetails.fat +
                  4 * orderedProductDetails.protein
                )
                  .toString()
                  .substring(0, 3)}  Cal`}
          ...
          <View
            style={{
              borderBottomWidth: 1,
              marginHorizontal: 10,
              borderBottomColor: 'rgba(196,202,204, 0.5)',
            }}
          />

          <View style={styles.cardsViewContainer}>
            <View style={{flexDirection: 'row', paddingHorizontal: 5}}>
              <Text style={{fontSize: 20, fontWeight: 'bold', flex: 0.7}}>
                Selected Location
              </Text>
              <Button
                onPress={() => this.onChangeLocaion()}
                transparent
                style={{
                  flex: 0.3,
